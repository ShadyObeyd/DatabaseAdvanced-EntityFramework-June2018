namespace MiniORM
{
    ...
    public abstract class DbContext
    {
            ...
            where TDbSet : class, new() where TCollection : class, new()
        {
            ...
            if (isManyToMany)
            {
                primaryKey = collectionType.GetRuntimeProperties().First(pi => collectionType.GetProperty(pi.GetCustomAttribute<ForeignKeyAttribute>().Name).PropertyType == entityType);
            }
            ...
            foreach (TDbSet entity in dbSet)
            {
                ...
                TCollection[] navigationEntities = navigationDbSet.Where(ne => primaryKey.GetValue(ne).Equals(primaryKeyValue)).ToArray();
            ...
            where TEntity : class, new()
        {
            ...
            foreach (PropertyInfo foreignKey in foreignKeys)
            {
                ...
                PropertyInfo navigationPrimaryKey = navigationProperty.PropertyType.GetRuntimeProperties().First(pi => pi.HasAttribute<KeyAttribute>());

                foreach (TEntity entity in dbSet)
                {
                    object foreignKeyValue = foreignKey.GetValue(entity);

                    object navigationPropertyValue = ((IEnumerable<object>)navigationDbSet).First(cnp => navigationPrimaryKey.GetValue(cnp).Equals(foreignKeyValue));
